    ...
    <div className="services-page">
      <div className="left-filter">
        ...
        <div className="left-input">
          ...
          <div>
            <input type="checkbox" checked={services.qonaqlama} onChange={() => setServices({ ...services, qonaqlama: !services.qonaqlama })} />
            <label>{language === 'az' ? 'Qonaqlama' : 'Accommodation'}</label>
          </div>
          <div>
            <input type="checkbox" checked={services.metbex} onChange={() => setServices({ ...services, metbex: !services.metbex })} />
            <label>{language === 'az' ? 'Mətbəx' : 'Kitchen'}</label>
          </div>
          <div>
            <input type="checkbox" checked={services.turlar} onChange={() => setServices({ ...services, turlar: !services.turlar })} />
            <label>{language === 'az' ? 'Turlar' : 'Tours'}</label>
          </div>
          <div>
            <input type="checkbox" checked={services.kendteserrufatimehhsullari} onChange={() => setServices({ ...services, kendteserrufatimehhsullari: !services.kendteserrufatimehhsullari })} />
        ...
        {currentItems.map((item) => (
          <div className="service-option" key={item.id}>
            <div className="service-img">
              <img src={item.image_url} alt={item.name} onClick={() => handleServiceClick(item.id)} />
            </div>
